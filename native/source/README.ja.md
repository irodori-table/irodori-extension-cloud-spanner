...
このディレクトリは `irodori.cloud-spanner` のマイグレーションステージングエリアです。アクティブなネイティブエントリポイントは `src/lib.rs` にあり、共有ABIヘルパーは `src/abi.rs` に、エンジンの動作は `src/stub.rs` または `src/driver.rs` にあります。エンジンスペシフィックな接続/クエリ/メタデータコードは、これらのスナップショットからその動作モジュールに移動すべきです。コネクタのランタイム契約がデスクトップアプリに組み込まれるにつれてです。
